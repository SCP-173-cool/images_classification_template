...
class ResNet_v2(object):
    ...
    def __init__(self, input_tensor, block, layers_lst, channels_lst, classes=1000, thumbnail=False):
        ...
        self.init = tf.keras.initializers.he_normal(seed=None)

    def build_model(self):
        self.training = tf.placeholder_with_default(tf.constant(False), shape=None,  name="training_switch")
    ...
    def build_classifier(self, features):
        with tf.variable_scope("Classifier"):
            output = tf.layers.dropout(features, rate=0.3, training=self.training)
            outputs = tf.reduce_mean(output, reduction_indices=[1, 2], name="avg_pool")
            outputs = tf.layers.flatten(outputs, name='flatten')
            outputs = tf.layers.dropout(outputs, rate=0.5, training=self.training)
    ...
    def _start_block(self):
        with tf.variable_scope("start_block"):
            ...
            if self.thumbnail:
                output = tf.layers.conv2d(output, self.channels_lst[0], 3, padding='SAME', use_bias=False, kernel_initializer=self.init)
            else:
                output = tf.layers.conv2d(output, self.channels_lst[0], 7, strides=(2, 2), use_bias=False, kernel_initializer=self.init)
    ...
    def _basic_block(self, input_tensor, channels, strides, downsample=False, name='Untitled'):
        ...
        with tf.variable_scope("BasicBLock_{}".format(name)):
            ...
            out = tf.nn.relu(out)
            if downsample:
                residual = tf.layers.conv2d(out, channels, 1, strides=strides, use_bias=False, kernel_initializer=self.init)
            out = tf.layers.conv2d(out, channels, 1, strides=strides, use_bias=False, kernel_initializer=self.init)

            out = tf.layers.batch_normalization(out, training=self.training)
            out = tf.nn.relu(out)
            out = tf.layers.conv2d(out, channels, 3, padding='SAME', use_bias=False, kernel_initializer=self.init)
    ...
    def _bottle_neck(self, input_tensor, channels, strides, downsample=False, name='Untitled'):
        ...
        with tf.variable_scope("BottleNeck_{}".format(name)):
            ...
            out = tf.layers.batch_normalization(out, training=self.training)
            out = tf.nn.relu(out)
            out = tf.layers.conv2d(out, channels//4, 3, strides=strides, padding='SAME', use_bias=False, kernel_initializer=self.init)

            out = tf.layers.batch_normalization(out, training=self.training)
            out = tf.nn.relu(out)
            out = tf.layers.conv2d(out, channels, 1, use_bias=False, kernel_initializer=self.init)
            out - tf.layers.dropout(out, rate=0.1, training=self.training)
...
def get_resnet(model_name, device="cpu:0", num_classes=1000, training=False, thumbnail=False):
    ...
    def resnet(input_tensor, config=resnet_spec[model_name]):
        with tf.device('/{}'.format(device)):
            net = ResNet_v2(input_tensor, config[0], config[1], config[2], num_classes, thumbnail=thumbnail)
        return net
...
if __name__ == '__main__':
    ...
    a, b = sess.run([resnet.features, resnet.outputs])
